    ...
    const productItems = this.props.products.map(product => (
      ...
      <Card bg="white" border="secondary" key={product.id} >
        <Link to={`/product/${product.id}`}>
          {product.images.length > 0 && <Card.Img variant="top" src={`/images/${product.images[0]?.id}`} ></Card.Img>}
        </Link>
        <Card.Body>
          ...
          <div className="pt-2 m-0" style={{ textAlign: 'center' }}>
            <Button variant="secondary" id="addcart" ><img src={shoppingCart} alt="shopping_cart" style={{ width: '20%' }} /><b> В корзину</b></Button>
          </div>
        </Card.Body>
      </Card >
    ));
